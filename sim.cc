...
int main(int argc, char* argv[]){
   ...
   if(!strcmp(argv[1],"bimodal")){           //cmd line args for bimodal branch predictor
      if(argc == 6){
      M2 = atoi(argv[2]);
        ...
        if(BTB_hit==0){ //miss case 
          ...
          btb_miss=1;
            ...
            for(unsigned int vl=0;vl<BTB_ASSOC;vl++){
              if(btb_cache[btb_index][vl].t_valid_bit == 0 ){ //check for number of invalids
                  invalid++;
              }
            }
            ...
            if(invalid == 1){                                   //case when only one block is empty
              for(unsigned int i=0;i<BTB_ASSOC;i++){
                if(btb_cache[btb_index][i].t_valid_bit == 0 ){  //update tag,LRU_counter and valid_bit 
                  for(unsigned int j=0; j<BTB_ASSOC;j++){
          ...
          if(invalid != 0 && invalid != 1){ //case when multiple blocks are empty     
            ...
            for(unsigned int k=0;k<BTB_ASSOC;k++){
                if(btb_cache[btb_index][k].t_valid_bit == 0 && btb_cache[btb_index][k].t_LRU_counter > currentHighest){  //check which are empty and find the one with highest lru count
